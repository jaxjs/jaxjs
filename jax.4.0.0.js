/**
 * jax JavaScript Library 4.0.0a (http://www.jaxjs.org/)
 *
 * @link       https://github.com/jaxjslibrary/jaxjs
 * @category   jax
 * @author     Nick Sagona, III <contact@nolainteractive.com>
...
(function(window){
    ...
    /** Extend jax with basic attribute, value and data methods as well as collection filtering methods */
    jax.extend({
        /**
    ...
    /** Extend jax with ready and load methods */
    jax.extend({
        /**
    ...
    /** Extend jax with clone, append, prepend, and parent/child methods */
    jax.extend({
        cloned : null,
        /**
    ...
    /** Extend jax with CSS-based methods */
    jax.extend({
    ...
    /** Extend jax with mouse, touch and scroll event methods */
    jax.extend({
        /**
    ...
    /**
     * Extend jax with object event handler methods
     */
    jax.extend({
        /**
    ...
    /**
     * Extend jax with the effects and animation methods
     */
    jax.extend({
        /**
     ...
     * @returns {Object}
     */
    window.jax.color = function(color) {
    ...
    window.jax.startDrag = null;
    window.jax.stopDrag  = null;
    window.jax.tO        = null;
    window.jax.iN        = null;

    /** Create the cookie object under the main jax object */
    window.jax.cookie = {
        /**
    ...
    /** Create the storage object under the main jax object */
    window.jax.storage = {
        length : 0,
        /**
    ...
    /** Method to revert back after a "no conflict" call */
    window.jax.revert = function() {
        window.$ = window.jax;
        return window.jax;
    };

    /** Now shorthand time function */
    window.jax.now = function() {
    ...
    /**
     * Function to compare two software versions
     *
     * @param   {String} first
     * @param   {String} second
     * @returns {Number}
     */
    window.jax.compare = function(first, second) {
        var result;
        second = (second == null) ? window.jax.version : second;

        // Create the test version object with major, minor and patch properties.
        var secondAry = second.split('.');
        var secondVersion = {
            'major' : parseInt(secondAry[0]),
            'minor' : parseInt(secondAry[1]),
            'patch' : parseInt(secondAry[2])
        };

        // Create the check version object with major, minor and patch properties.
        var firstAry = first.split('.');
        var firstMinor = (firstAry[1] != undefined) ? parseInt(firstAry[1]) : 0;
        var firstPatch = (firstAry[2] != undefined) ? parseInt(firstAry[2]) : 0;
        var firstVersion = {'major' : parseInt(firstAry[0]), 'minor' : firstMinor, 'patch' : firstPatch };

        // Perform the comparison between the two versions.
        if (secondVersion.major > firstVersion.major) {
            result = 1;
        } else if (secondVersion.major < firstVersion.major) {
            result = -1;
        } else if (secondVersion.major == firstVersion.major) {
            if (secondVersion.minor > firstVersion.minor) {
                result = 1;
            } else if (secondVersion.minor < firstVersion.minor) {
                result = -1;
            } else if (secondVersion.minor == firstVersion.minor) {
                if (secondVersion.patch > firstVersion.patch) {
                    result = 1;
                } else if (secondVersion.patch < firstVersion.patch) {
                    result = -1;
                } else if (secondVersion.patch == firstVersion.patch) {
                    result = 0;
                }
    ...
    /**
     * Function to get the scroll bar size in the current browser
     *
     * @returns {Number}
     */
    window.jax.getScrollBarSize = function() {
        var inner = document.createElement('p');
        var outer = document.createElement('div');

        window.jax(inner).css({
            width  : '100%',
            height : '200px'
        });

        window.jax(outer).css({
            position   : 'absolute',
            top        : '0px',
            left       : '0px',
            visibility : 'hidden',
    ...
    /** Function to get variables passed into JS files */
    window.jax.getVars = function() {
    ...
    window.jax.buildQuery = function(data) {
        ...
        // Loop through the elements to assemble the POST parameters.
     ...
     * @returns {Array}
     */
    window.jax.parseOptions = function(file, values) {
...
/** Function to detect jaxRoot and any script vars*/
jax.getVars();
jax(document).ready(jax.getVars);
